...
class PPOBuffer:
    ...
    def finish_trajectory(self, last_value):
        ...
        advantages = np.zeros_like(rewards, dtype=np.float32) # Advantages is initialized as a zero array of shape rewards which stores the computed advantage for each time step
        gae = 0 #Gae is a temporary accumulator variable which holds the running sum for the GAE calculation

        #loop over time steps in REVERSE. (t= N-1 to t = 0)
        for t in reversed(range(len(rewards))):
            delta = rewards[t] + self.gamma * values[t+1] * (1 - dones[t]) - values[t] #delta is the Temporal Difference Error for time step t
    ...
    def generate_batches(self):
        ...
        '''
        print("States tensor shape:", states.shape)  # Should output: (N, 405)
